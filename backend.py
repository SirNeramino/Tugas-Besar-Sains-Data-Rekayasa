...
class MLWrapper:
    """Wrapper untuk algoritma standar"""
...
def load_real_data(dataset_name):
    try:
        if "Performa Turbin" in dataset_name:
            ...
            # Gunakan dataset klasifikasi anomali yang baru
            df = pd.read_csv('turbine_gearbox_anomaly.csv')
            df = df.loc[:, ~df.columns.str.contains('^Unnamed')]
            return df.dropna()
...
def run_regression_fungsi1(metode, parameters, df):
    ...
    if metode == "SARIMA (Time-Series)":
        X, y, time_series = X.tail(2000), y.tail(2000), time_series.tail(2000)
    elif metode == "Recurrent Neural Network (RNN)":
        X, y, time_series = X.tail(10000), y.tail(10000), time_series.tail(10000)
    ...
    if metode == "SARIMA (Time-Series)":
        model_fit = SARIMAX(endog=y_train, exog=X_train, order=(1, 1, 1)).fit(disp=False)
        ...
        raw_model = XGBRegressor(n_estimators=100, learning_rate=0.1, random_state=42, n_jobs=-1).fit(X_train_scaled, y_train)
        y_pred, model = raw_model.predict(X_test_scaled), MLWrapper(raw_model, scaler)
        feat_importance = pd.DataFrame({'Fitur': features, 'Tingkat Kepentingan': raw_model.feature_importances_})
        
    elif metode == "Random Forest":
        raw_model = RandomForestRegressor(n_estimators=100, random_state=42, n_jobs=-1).fit(X_train_scaled, y_train)
        y_pred, model = raw_model.predict(X_test_scaled), MLWrapper(raw_model, scaler)
        feat_importance = pd.DataFrame({'Fitur': features, 'Tingkat Kepentingan': raw_model.feature_importances_})
        
    elif metode == "Multi-Layer Perceptron (MLP)":
        raw_model = MLPRegressor(max_iter=parameters.get('epochs', 200), random_state=42).fit(X_train_scaled, y_train)
        y_pred, model = raw_model.predict(X_test_scaled), MLWrapper(raw_model, scaler)
        
    else: 
        raw_model = SVR(kernel='rbf', C=100, gamma=0.1, epsilon=.1).fit(X_train_scaled, y_train)
        y_pred, model = raw_model.predict(X_test_scaled), MLWrapper(raw_model, scaler)
    ...
    chart_df = pd.DataFrame({
        ...
        'Galat (Residual)': y_test.values - y_pred
    })
    ...
    return {
        ...
        "status": "Sukses", "chart_data": chart_df, "feature_importance": feat_importance,
        "model": model, "fitur": features         
    }

# ==========================================
# FUNGSI 2: KLASIFIKASI ANOMALI GEARBOX
# ==========================================

def run_anomaly_fungsi2(metode, parameters, df):
    # Tentukan fitur fisis gearbox
    features = ['gearbox_oil_temp', 'gearbox_bearing_temp', 'vibration_x', 'vibration_y', 'vibration_z', 'oil_pressure', 'particle_count']
    X = df[features]
    y = df['is_anomaly'] # 0 = Normal, 1 = Anomaly
    
    train_ratio = parameters.get('train_size', 80) / 100.0
    
    # Untuk klasifikasi anomali, kita gunakan data splitting biasa (stratified) agar proporsi anomali terjaga
    X_train, X_test, y_train, y_test = train_test_split(X, y, train_size=train_ratio, stratify=y, random_state=42)
    
    # Normalisasi Data (Sangat krusial untuk SVM dan KNN)
    scaler = StandardScaler()
    X_train_scaled = scaler.fit_transform(X_train)
    X_test_scaled = scaler.transform(X_test)
    ...
    # Percabangan Algoritma Klasifikasi
    if metode == "Random Forest Classifier":
        raw_model = RandomForestClassifier(n_estimators=100, random_state=42, n_jobs=-1)
        raw_model.fit(X_train_scaled, y_train)
        y_pred = raw_model.predict(X_test_scaled)
        model = MLWrapper(raw_model, scaler)
        feat_importance = pd.DataFrame({'Fitur': features, 'Tingkat Kepentingan': raw_model.feature_importances_})
        
    elif metode == "Support Vector Classifier (SVC)":
        raw_model = SVC(kernel='rbf', probability=True, random_state=42)
        raw_model.fit(X_train_scaled, y_train)
        y_pred = raw_model.predict(X_test_scaled)
        model = MLWrapper(raw_model, scaler)
        
    elif metode == "K-Nearest Neighbors (KNN)":
        # Gunakan K=5 sebagai baseline
        raw_model = KNeighborsClassifier(n_neighbors=5, n_jobs=-1)
        raw_model.fit(X_train_scaled, y_train)
        y_pred = raw_model.predict(X_test_scaled)
        model = MLWrapper(raw_model, scaler)
    ...
    # Buat dataframe untuk matriks kebingungan (Confusion Matrix) agar mudah di-plot di frontend
    ...
    return {
        "metrik1_nama": "Accuracy", "metrik1_nilai": f"{acc*100:.2f}%",
        "metrik2_nama": "Precision", "metrik2_nilai": f"{prec*100:.2f}%",
        "metrik3_nama": "Recall (Sensitivitas)", "metrik3_nilai": f"{rec*100:.2f}%",
        "metrik4_nama": "F1-Score", "metrik4_nilai": f"{f1*100:.2f}%",
        "status": "Sukses",
        "cm_data": cm_df,
